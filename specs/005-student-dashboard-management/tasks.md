...
- [X] T001 Change `app/models.py`: `Company` gains `overview` (nullable Text); `Application` gains
  `interview_mode` (nullable String) and `company_remark` (nullable Text). Per data-model.md.
- [X] T002 Autogenerate the migration (`flask db migrate`), review it matches data-model.md exactly,
  apply it (`flask db upgrade`) — depends on T001.
- [X] T003 [P] Add a sample `Company.overview` value to `data-seeds/seed_data.py`'s seeded Company —
  depends on T001.
- [X] T004 [P] Remove the Milestone 2 `GET /api/student/ping` placeholder route from
...
- [X] T005 [US1] Implement `GET /api/student/profile` in `app/routes/student.py`: returns the
  caller's own `Student` fields plus `photo_url`/`resume_url` via `app/utils.py`'s `static_url()` —
  depends on T004.
- [X] T006 [US1] Implement `POST /api/student/profile` in `app/routes/student.py`: accepts
...
- [X] T007 [US1] Create `frontend/src/views/StudentHome.vue` (replacing the Milestone 2 ping
...
- [X] T008 [US1] Verify per `quickstart.md` → "US1" section: every field persists; the uploaded
...
- [X] T009 [US2] Implement `GET /api/student/organizations` in `app/routes/student.py`: `Company`
  query filtered to `approval_status == "approved"`, optional `q` on `company_name` — depends on
  T004.
- [X] T010 [US2] Implement `GET /api/student/organizations/<id>` in `app/routes/student.py`: `404`
  if not found or not approved, else `overview`/`logo_url`/`industry`/`location` — depends on T004.
- [X] T011 [US2] Implement `GET /api/student/drives` in `app/routes/student.py`: always filtered to
...
- [X] T012 [US2] Implement `GET /api/student/drives/<id>` in `app/routes/student.py`: no status
...
- [X] T013 [US2] In `StudentHome.vue`, add "Organizations" (from T009) with a "view details" per row
  ...
  T012, T007.
- [X] T014 [US2] Verify per `quickstart.md` → "US2" section: only approved Companies appear; only a
...
- [X] T015 [US3] Implement `POST /api/student/drives/<id>/apply` in `app/routes/student.py`: `404`
...
- [X] T016 [US3] In the Drive modal (T012/T013), show an "Apply" button when `!already_applied`,
  calling T015 and reloading the modal's data on success — depends on T015, T013.
- [X] T017 [US3] Verify per `quickstart.md` → "US3" section: first Apply succeeds (`201`); second
...
- [X] T018 [US4] Implement `GET /api/student/applications` in `app/routes/student.py`: every
  ...
  depends on T004.
- [X] T019 [US4] Extend `POST /api/company/applications/<id>/decision` in `app/routes/company.py`:
...
- [X] T020 [US4] Extend `POST /api/company/applications/<id>/interview` in `app/routes/company.py`:
...
- [X] T021 [US4] Extend `GET /api/company/drives/<id>/applications` in `app/routes/company.py`:
  optional `status` filter and `sort=status` query params (FR-011) — depends on nothing new.
- [X] T022 [US4] In `CompanyHome.vue`'s Application-detail modal, add a `remark` text input (calling
...
- [X] T023 [US4] In `CompanyHome.vue`'s Applications-list modal, add a status filter dropdown and a
  "sort by status" toggle, re-fetching T021 — depends on T021.
- [X] T024 [US4] In `StudentHome.vue`, add "Applied Drives" (dashboard summary, from T018) and a
...
- [X] T025 [US4] Verify per `quickstart.md` → "US4" section and "FR-011" section: Company sets a
...
- [X] T026 [US5] Add a search input to `StudentHome.vue`'s Organizations/Drives area, calling T011
...
- [X] T027 [US5] Verify per `quickstart.md` → "US5" section: search by Company/title/skills returns
...
- [X] T028 [US6] Implement `GET /api/student/placement/confirmation` in `app/routes/student.py`:
  ...
  T004.
- [X] T029 [US6] In `StudentHome.vue`, show a "Download Placement Confirmation" button (calling
  T028) only if `GET /api/student/applications` or a dedicated check shows a Placement exists —
  depends on T028, T007.
- [X] T030 [US6] Verify per `quickstart.md` → "US6" section: download succeeds with correct content
...
- [X] T031 Verify per `quickstart.md` → "Role check" section: a Company session gets `403` on every
  `/api/student/*` endpoint; no session gets `401` — depends on T005, T009, T010, T011, T012, T015,
  T018, T028.
- [X] T032 Re-run the full `quickstart.md` end-to-end (backend `curl` pass, then the frontend smoke
  test) against a freshly reseeded database and confirm every SC-00x in spec.md still holds.
- [X] T033 Commit with the milestone-specific message required by the constitution and push.
