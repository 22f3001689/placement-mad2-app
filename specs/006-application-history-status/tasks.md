...
**Checkpoint**: Foundation ready — `Application.status` values are now `applied/shortlisted/interview/offer/rejected/placed` everywhere in the DB, and seed data matches the new vocabulary.
...
- [ ] T005 [US1] In `app/routes/company.py`, change `APPLICATION_DECISION_STATUSES` to `("shortlisted", "interview", "offer", "rejected", "placed")` and update the `400` error message to match
- [ ] T006 [US1] In `app/routes/company.py`'s `decide_application`, add a terminal-status guard: if `application.status` is already `placed` or `rejected`, return `409` before applying any change (per contracts/company-api.md)
...
- [ ] T008 [P] [US1] In `frontend/src/views/CompanyHome.vue`, update the status `<select>` options in the "Student Application" modal to the six new values (Applied/Shortlisted/Interview/Offer/Rejected/Placed → `applied/shortlisted/interview/offer/rejected/placed`), and add inline inputs for position title/salary/joining date plus a "Confirm Placement" action that only appears/applies when `placed` is selected, calling `setStatus` with the extra fields
...
- [ ] T011 [P] [US2] In `frontend/src/views/StudentHome.vue`'s History modal table, add a "Placement" column/row that shows `a.placement.position_title` (and salary/joining date) when `a.placement` is present, otherwise blank
...
- [ ] T015 [P] [US3] In `frontend/src/views/AdminHome.vue`, add a "View Profile" button per row in the "Registered Students" section that fetches `GET /admin/students/<id>` and opens a new modal showing the full profile fields and application history table
